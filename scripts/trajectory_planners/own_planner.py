...
def plan_paths_own(json_data):
    ...
    generate_req.rotations_per_cell = int(json_data["rotations-per-cell"] if 'rotations-per-cell' in json_data else 1)
    ...
    generate_req.min_sub_polygons_per_uav = int(json_data['min-pieces-per-uav'] if 'min-pieces-per-uav' in json_data else 1)
    ...
    generate_req.override_drone_parameters = False
    generate_req.override_battery_model = False
