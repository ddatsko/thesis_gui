...
EXPERIMENTS_LOG_FILE_HEADER = ['algorithm', 'energy', 'path_time', 'path_length', 'n_turns', 'computation_time', 'max_energy', 'min_energy', 'n_paths']
...
def compare_algorithms(config):
    ...
    for experiment in config['experiments']:
        for algorithm in config['algorithms']:
            ...
            json_data = read_config(config['experiments_dir'] + '/' + experiment)
            ...
            if experiment in config.keys() and 'start_angle' in config[experiment] and 'angle_step' in config[experiment]:
                paths, res = test_algorithm_many_times(json_data, algorithm, config['start_angle'], config['angle_step'])
            paths, res = run_one_algorithm(json_data, algorithm_function)
